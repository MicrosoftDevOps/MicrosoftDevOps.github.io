...
## Customer profile ##
ExakTime eliminates the need for handwritten paper time cards at job sites with innovative time-tracking solutions. Our Hackfest focused on their core customer front-end services and back-end APIs, which run their time-tracking software. The time-tracking software all run as Azure App services.
...
1. Be able to build, test, and release the app services independently. This significantly reduced the build times to around 10 minutes from 45-60 minutes. It enables faster feedback and opens up the possibility of deploying independently and breaking apart the code base into separate Git repositories.
...
3. Run the new Git Merge task nightly. In the transition to one branch and separate Git repositories for each of the services, Microsoft created a "git merge" VSTS task that tests to see if any merge conflicts will happen if those branches merge individually or all together into their higher-level branch.  
