...
1. Be able to build all 10 of the app services independently - This signficantly reduced the build times down to around 5-10 minutes<@customer-confirm> from the 45 min - 1 hour, enables faster feedback, and lights up the possibility to deploy independently and break apart the code base into separate git repositories.
...
<@Insert Screenshot of new branch structure>

3. Run the new Git Merge task nightly - In the transition to one branch and separate git repositories for each of the 10 services, Microsoft created a "git merge" VSTS task which tests to see if any merge conflicts will happen if those branches merged individually or all together into their higher-level branch.  
<@Insert Screenshot of VSTS git merge Task>  
...
4. Setup a deployment and release pipeline based on complimentary service pairs - Developers usually would work on a complimentary front-end and corresponding back-end API app service together and this has the ability to delivery end-user value in it self.  For these reasons it makes sense to break apart the "ship all 10 services at once" mentality and shift them to be deployed more independently.
<@Insert Screenshot of new branch + release structure>
...
<@Insert Screenshot of continuous deployment trigger>
<@Insert Screenshot of release tasks/pipeline>
