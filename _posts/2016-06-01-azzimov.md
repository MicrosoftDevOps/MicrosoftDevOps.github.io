...
Azzimov was having quality issues when deploying new releases: Due to the lack of automated tests, they had limited capabilities to ensure a desired quality of their product before pushing it in production. This is a very usual situation in small to medium companies.
As a secondary objective, Azzimov also wanted to reduce the time to market of new features.
One of the main pain of Azzimov was the lack of tooling around DevOps Practices, they clearly identified their hope to improve this point during the hackfest.
...
This exercise allowed us to quickly understand the main issue: There is no quality check in the process except some sparse manual testing. Defects could be passed downstream to the production: releasing is thus very risky.
...
From the build to the release, each steps was manual and done by different people in the company without any official process. The build process (using Maven 2) was executed manually from a developer machine.
