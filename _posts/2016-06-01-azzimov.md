...
One of the main pain that we identified during the Value Stream Mapping is the lack of tooling to help Azzimov implementing some DevOps Practices.

From the build to the release, each steps were manual and did locally by different people in the company without any official process.
For the build process, it was done locally from one developer machine using Apache Maven 2 when the team decided to did it.

During the Hackfest we explained the concept of the Continuous Integration and the availability to build the code at each commit in the GitHub repository, to the impact to add units testing and run it during the Build process.

We decided to automated those tasks and created a Pipeline project in Jenkins.

We identified four tasks:
* Checkout Sources : Classic git clone from the repository
* Build : run the shell command "mvn clean install -DskipTests" to build the .Jar file
