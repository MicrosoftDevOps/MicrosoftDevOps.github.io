...
From the previous build that we had just created, we had the .exe solution plus the deployment files for the Cloud Services on Azure, which we called the “artifacts.”
...
3. Deploy our new Cloud Services using the deployment file that we customized just one step earlier.
  * When the Cloud Services is deployed on Azure, the virtual machine inside calls a bootstrap script (.cmd) that downloads and installs the .exe solution on it.
